...
readmem:   
   lcall getbyt           ; get address high byte
   mov   dph, a           ; push msb of jump address into dptr
   lcall prthex			  ; print top byte
   lcall getbyt           ; get address low byte
   mov   dpl, acc         ; push lsb of jump address into dptr
   lcall prthex			  ; print low byte
   lcall crlf			  ; new line
   movx  a, @dptr		  ; move value at location in dptr to acc
   lcall prthex			  ; print stored value
   lcall crlf
   ljmp  endloop          ; return
   
;===============================================================
; command writemem  'w'
; this command reads in an address and a byte of data from the 
...
writemem:
   ...
   mov   dpl, acc         ; push lsb of jump address into dptr
   lcall prthex			  ; print low byte
   lcall getchr			  ; get the equals sign
   cjne  a, #3Dh, noequal ; jump if that was not an equals sign
   lcall sndchr			  ; print symbol
   lcall getbyt			  ; get the data to store
   movx  @dptr, a		  ; move data from acc to address
   lcall prthex			  ; print data
noequal:
   lcall crlf			  ; new line
   ljmp  endloop          ; return
